...
const initialProducts: Product[] = [
  { id: 'prod-1', name: 'กาแฟเอสเปรสโซ่', price: 55, stock: 100, sku: 'SKU001', description: 'ช็อตกาแฟเข้มข้นสกัดจากเมล็[...]\u200b', imageUrl: '', type: ProductType.INVENTORY, category: 'เครื่องดื่ม' },
  { id: 'prod-2', name: 'คาปูชิโน่', price: 65, stock: 80, sku: 'SKU002', description: 'กาแฟผสมนมและฟองนมเนียนนุ่ม รสชาต[...]\u200b', imageUrl: '', type: ProductType.INVENTORY, category: 'เครื่องดื่ม' },
  { id: 'prod-3', name: 'ครัวซองต์เนยสด', price: 75, stock: 50, sku: 'SKU003', description: 'ครัวซองต์อบใหม่หอมกรุ่น ทำจ[...]\u200b', imageUrl: '', type: ProductType.INVENTORY, category: 'เบเกอรี' },
  { id: 'prod-4', name: 'น้ำส้มคั้นสด', price: 60, stock: 45, sku: 'SKU004', description: 'สดชื่นด้วยน้ำส้มคั้นสด 100% ไม่ผ�[...]\u200b', imageUrl: '', type: ProductType.INVENTORY, category: 'เครื่องดื่ม' },
  { id: 'prod-5', name: 'ชามะนาว', price: 50, stock: 90, sku: 'SKU005', description: 'ชาไทยรสเข้มข้นผสมผสานกับความเปรี[...]\u200b', imageUrl: '', type: ProductType.INVENTORY, category: 'เครื่องดื่ม' },
  { id: 'prod-6', name: 'บราวนี่ช็อคโกแลต', price: 80, stock: 30, sku: 'SKU006', description: 'บราวนี่เนื้อหนึบ เข้มข้��[...]\u200b', imageUrl: '', type: ProductType.INVENTORY, category: 'เบเกอรี' },
];
...
export const InventoryProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    // Initialize sql.js, explicitly providing the path to the wasm file to prevent loading errors in the browser.
    initSqlJs({
  ...
  const disconnectDb = useCallback(() => {
    ...
    setDb(null);
    setDbFileName(null);
    // Force reload from initial state / localStorage
    window.location.reload();
  }, [db]);

  useEffect(() => {
    if (db || !SQL) return; // Do not load from localStorage if DB is connected or SQL not ready
