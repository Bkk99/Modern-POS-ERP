...
const ProductForm: React.FC<{ product: Partial<Product> | null; onSave: (product: any) => Promise<void>; onCancel: () => void }> = ({ product, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!dataToSave.imageUrl) {
        dataToSave.imageUrl = `https://source.unsplash.com/400x400/?${encodeURIComponent(dataToSave.name || 'product')}`;
    }
    try {
